...
void AST::mainReport(const std::vector<std::string>& profileName) {
    ...
    for(std::list<AST*>::iterator i = child.begin(); i != child.end(); i++) {
        AST* test = nullptr;
        if((*i)->tag == "function") {
            test = (*i)->getChild("name");
            if(test->getName() == "main") {
                AST* blk = (*i)->getChild("block")->getChild("block_content");
                AST* rtn = blk->getChild("return");
                std::list<AST*>::iterator rtnIt = rtn->child.begin();

                for(std::vector<std::string>::const_iterator j = profileName.begin(); j != profileName.end(); j++) {
                    std::string out = "std::cout << " + *j + " << std::endl;\n";
                    AST* code = new AST(token, out);
                    rtn->child.insert(rtnIt, code);
                }
            }
        }
    }
...
void AST::functionCount(const std::string& profileName) {
    ...
    for(std::list<AST*>::iterator j = child.begin(); j != child.end(); j++) {
        ((*j)->functionCount(profileName));
    }

    for(std::list<AST*>::iterator i = child.begin(); i != child.end(); i++) {
        if((*i)->tag == "function" || (*i)->tag == "constructor" || (*i)->tag == "destructor") {
            std::string funcName = (*i)->getChild("name")->getName();
            std::string out = " " + profileName + ".count(__LINE__, \"" + funcName + "\");\n";
            AST* blk = (*i)->getChild("block")->getChild("block_content");
            AST* code = new AST(token, out);
            blk->child.insert(blk->child.begin(), code);
        }
    }
}
